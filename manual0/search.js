...
*	History : 
* -----------------------------------------------------------
*	13-Feb-2015      P-30-02     $$1     rkumbhare 	Created.
*	04-Jul-2015      P-30-12     $$2     rkumbhare  Updated.
*	31-Aug-2015	 P-30-15     $$3     rkumbhare  Updated.
*       04-Dec-2015      P-30-22     $$4     rkumbhare  Updated.
*       16-Jun-2016      P-30-34     $$5     rkumbhare  Updated.
*
*
...
{	
  ...
  this.OnSearchFieldChange = function(evt) {
    ...
    var searchValue = this.DOMSearchField().value.replace(/ +/g, ""); // strip whitespaces
...
var statusChkDMA = false;
var statusGlobal = false;
var statusCompClass = false;
var statusException = false;
var statusEnumeration = false;
var statusLicensed = false;
var added = 0;
  ...
  this.OnSelectItem = function(id) {
   ...
   statusChkDMA = document.getElementById("DMAMethod").checked;
   statusGlobal = document.getElementById("GlobalMethod").checked;
   statusCompClass = document.getElementById("CompClsMethod").checked;
   statusException = document.getElementById("Exception").checked;
   statusEnumeration = document.getElementById("Enumeration").checked;
   statusLicensed = document.getElementById("LicenseMethod").checked;
...
*/
  ...
  this.Search = function() {
    this.keyTimeout = 0;	    
    var searchValue = this.DOMSearchField().value.replace(/^ +/, ""); // strip leading whitespace
...
*/	
    ...
    for (var i = 0; i < searchData.length; i++)  {      
        ...
        if(statusChkDMA == true)
		{ // Searching in DMA enabled method name
			if ((regex.test (searchData[i][1][0]))   ){     
				if(searchData[i][1][1][2] == "PMA_DMA")
					results_entry.push(i);            
			}
		} else if(statusGlobal == true){
			if ((regex.test (searchData[i][1][0])) ){	// assuming in case of global method.
				if(searchData[i][1][1][3] == "global")
					results_entry.push(i);            
			}
		} else if(statusCompClass == true){
			// Searching in compact class name
			if ((regex.test (searchData[i][0]))   ){     
				if(searchData[i][1][1][3] == "compact")
					results_entry.push(i);            
			}
			// Searching in compact method name
			if ((regex.test (searchData[i][1][0]))   ){     
				if(searchData[i][1][1][3] == "compact")
					results_entry.push(i);            
			}
		} else if(statusLicensed == true){
			// Searching in class name not required as license is applied to methods
			// Searching in method name for license
			if ((regex.test (searchData[i][1][0]))   ){     
				if(searchData[i][1][1][4] == "licensed")
					results_entry.push(i);            
			}
		} else if(statusException == true){
			if ((regex.test (searchData[i][0])) ){	// assuming in case of exception.
				if(searchData[i][1][1][3] == "exception"){
					results_entry.push(i);            
					added = 1;
				}
			}
		...
		} else if(statusEnumeration == true){
			// Searching in class name 
			if ((regex.test (searchData[i][0])) ){	// assuming in case of enum method name as "".
				if(searchData[i][1][1][3] == "enum")
					results_entry.push(i);            
			}
		} else {
			// Searching in class name
		    if ((regex.test(searchData[i][0]))) {
		        ...
		        if (searchData[i][1][1][3] == "") {
		            results_entry.push(i);
		            added = 1;//Set added to 1 to avoid duplicate entries in search results
		        }			
			}
...
function createResults(results_entry) 
{
   ...
   for (var j = 0; j < results_entry.length; j++) {    
	...
	var apiName = null;
	var apiLink;
	if (searchData[e][1][1][3] == "enum" || searchData[e][1][1][3] == "global")
	{
	 apiName = searchData[e][0];
	 apiLink = searchData[e][1][1][0];	
	} else {	    
		if (searchData[e][0] != "" && searchData[e][1][0]!= "") {
